...
MA Optimizer — TUNED
MA windows: [3, 5, 10]
  - MA(10) added to capture medium-term momentum in GLD, VNQ, SLV
    which trend on longer timeframes than TLT/VCIT
  - targets now dict with 'train'/'val'/'test' keys
  - predict_single_etf called with source_feature_dict for API compatibility
...
MA_WINDOWS = [3, 5, 10]   # MA(10) added for GLD/VNQ/SLV medium-term momentum
...
def optimize_ma_window(
    ...
    for ma_window in MA_WINDOWS:
            ...
            for etf in etf_list:
                if etf in X_window and X_window[etf].shape[0] > 0:
                    tv_preds = model.predict_single_etf(
                        X_window[etf],
                        etf,
                        source_feature_dict=X_window,
                    )
                    preds_dict[etf] = pd.Series(tv_preds, index=X_window[etf].index)
            return preds_dict
        ...
        if val_preds:
            ...
            val_bt = run_backtest(
                val_preds, price_aligned, tbill_aligned, engine_val, val_common)
            val_metrics = calculate_metrics(
                val_bt["equity_curve"]["strategy"],
                val_bt["returns"], val_bt["risk_free"], val_bt["audit_trail"])
        ...
        if oos_preds:
            ...
            oos_bt = run_backtest(
                oos_preds, price_aligned, tbill_aligned, engine_oos, oos_common)
            oos_metrics = calculate_metrics(
                oos_bt["equity_curve"]["strategy"],
                oos_bt["returns"], oos_bt["risk_free"], oos_bt["audit_trail"])
