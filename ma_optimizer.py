...
def optimize_ma_window(
    ...
    for ma_window in [3, 5]:
        ...
        # --------------------------------------------------
        # 1. Determine Common Date Range Across All ETFs
        # --------------------------------------------------
        ...
        for etf in etf_list:
            ...
            etf_dates = features_dict[etf].index

            if common_dates is None:
                common_dates = etf_dates
            else:
                common_dates = common_dates.intersection(etf_dates)

        common_dates = pd.DatetimeIndex(sorted(common_dates))

        # Align price + tbill strictly to common universe
        price_aligned = price_df.loc[common_dates]
        tbill_aligned = tbill_df.loc[common_dates]
        ...
        for etf in etf_list:
            ...
            predictions_dict[etf] = pd.Series(
                preds,
                index=test_dates
            )
