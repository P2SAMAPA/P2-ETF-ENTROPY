...
1. Cross-sectional ETF selection
...
class StrategyEngine:
    ...
    def generate_signals(self, predictions_dict, price_df, dates):
        ...
        for date in dates:
            ...
            # ------------------------------------------------
            # 1. Apply Trailing Stop (only if in ETF)
            # ------------------------------------------------
            if previous_position not in [None, "CASH"]:

                if date in price_df.index:

                    price_today = price_df.loc[date, previous_position]
            ...
            # ------------------------------------------------
            # 4. Re-entry from cash (Z-score filter)
            # ------------------------------------------------
            ...
            # ------------------------------------------------
                ...
                elif best_etf != previous_position:

                    gain_diff = best_ret - expected_dict.get(previous_position, 0)
            ...
            # ------------------------------------------------
            # Detect switch cleanly
            # ------------------------------------------------
            switch_flag = selected != previous_position
            ...
            signals.append({
                ...
                "z_score": None if best_etf is None else self.compute_z_score(
